...
# Exclude hev-jni.c since we use our own JNI wrapper
SRCFILES_FILTERED := $(filter-out $(SRCDIR)/hev-jni.c,$(SRCFILES))
LOCAL_SRC_FILES := $(patsubst $(SRCDIR)/%,src/%,$(SRCFILES_FILTERED))
LOCAL_C_INCLUDES := \
	$(LOCAL_PATH)/src \
	$(LOCAL_PATH)/src/misc \
